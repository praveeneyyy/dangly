...
Dangly is an open source project. There is one supported version at a time:
the latest release.
...
Dangly runs entirely on your device. It makes no network calls, has no server, no
